...
const docsImages = [JS_IMAGE_FILENAME, CSS_IMAGE_FILENAME];

const modifiedImages = cp
  .spawnSync('git', ['diff HEAD~1 HEAD', '--name-status', '--cached'], {
    encoding: 'utf8',
  })
  .output[1].split('\n')
  .filter((filePath) =>
    docsImages.find((imgName) => filePath.includes(imgName))
  );
